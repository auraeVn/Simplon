...
<body>
    ...
    <div class="container">
        ...
        try {
            ...
            $requete = $bd->prepare('SELECT DISTINCT localite FROM fournisseurs ORDER BY localite ');
            $requete->execute();
        ?>


            <form class="formConnexAjout" method="post" action="traitementRechercheFournisseursVille.php">

                <div class="formDiv2">
                    <label class="labelForm" for="nom">Recherche de fournisseur par Ville</label><br>
                    <?php
                    echo '<select class="form" required="required" placeholder="Nom" type="text" name="fournisseurVille">';
                    while ($obj = $requete->fetch(PDO::FETCH_OBJ)) {
                        echo '<option value="' . $obj->localite . '">' . $obj->localite . '</option>';
                    }
                    echo '</select>';
                    ?>
        ...
        $Id = $_GET['id'];
        $fournisseurVille = $_POST['fournisseurVille'];

        try {

            $requete = $bd->prepare("SELECT * FROM fournisseurs WHERE localite = '$fournisseurVille'");
            $requete->execute();
